...
```{r}
# est_loop=loop( [specify Y, Tr, Z (i.e. covariates), and
# p (probability of assignment to treatment)] )
```
...
The call is the same as before, except we use the `pred=reloop` prediction algorithm, and 
include `yhat=yhat` as an additional argument:
```{r}
## fill in:
# est_reloop <- 
...
```
Then re-run the estimation routine: (fill in)
```{r}
# est_reloop <- 
...
save(abdat, covariates,est_reloop,files='abResults.RData')
```
