---
title: "RCT Treatment Effect Estimation with `dRCT` Package"
date: "`r Sys.Date()`"
...
The `dRCT` package is currently hosted on Github at [manncz/dRCT](https://github.com/manncz/dRCT). To install it, use the `install_github()` function from the `devtools` package.
...
Use `set.seed()` to have a reproducible results if using random forests to impute potential outcomes. This can take a couple of seconds to run. 
```{r}
set.seed(483)
