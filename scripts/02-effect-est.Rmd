...
needed.packages <- c("dplyr", "devtools")
...
Then, we can input our matrix of pre-treatment covariates into the `loop` function. This can take a couple of seconds to run.
```{r}
loop(Y = rct_dat$taks08, Tr = rct_dat$TrBern, Z = covMat, pred = loop_rf)
```

Compare these to the difference in means estimator. This estimator is the estimated coefficient and standard error for `TrBern` in the following OLS regression:
